...
mod tests {
    ...
    /// Runs validation expecting a panic and returns its message.
    fn validation_panic_message(args: &GenerateArgs) -> String {
    ...
    fn remote_spammer_requires_relayer() {
        let mut args = generate_args();
        args.spammer = true;
        assert!(validation_panic_message(&args).contains("--spammer requires --relayer"));
    }
    ...
    fn remote_channel_fraction_requires_spammer() {
        ...
        assert!(
            validation_panic_message(&args)
                .contains("--spammer-channel-fraction > 0 requires --spammer")
        );
    }
