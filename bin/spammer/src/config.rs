...
pub fn resolve_named_http_url(url: &str, hosts_path: Option<&Path>) -> String {
    ...
    let hosts: DeployerHosts = serde_yaml::from_str(&raw).expect("failed to parse hosts file");
...
"#,
    ...
    #[test]
    fn named_relayer_url_accepts_monitoring_ip_pair_hosts_file() {
        let hosts_yaml = r#"monitoring:
...
"#;
        let hosts_path =
            std::env::temp_dir().join(format!("spammer-hosts-ip-pair-{}.yaml", std::process::id()));
        std::fs::write(&hosts_path, hosts_yaml).expect("write hosts");

        let resolved = resolve_named_http_url("http://relayer-node:8080", Some(&hosts_path));

        assert_eq!(resolved, "http://203.0.113.9:8080");
