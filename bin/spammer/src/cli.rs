...
pub struct Cli {
    ...
    /// Value to transfer per transaction (must be > 0 and <= the warm-up
    /// mint amount every ring account starts with, RING_MINT_AMOUNT).
    #[arg(long, default_value_t = 1)]
    pub value: u64,
    ...
    /// Fraction of submitter iterations that start a payment-channel
    /// lifecycle (open -> stream off-chain vouchers -> close) instead of
    /// submitting a transfer batch. Lifecycles run concurrently, so transfers
    /// keep flowing while channels open and settle. `0` (default) disables
    ...
    #[arg(long, default_value_t = crate::config::DEFAULT_CHANNEL_VOUCHERS, value_parser = clap::value_parser!(u64).range(1..))]
