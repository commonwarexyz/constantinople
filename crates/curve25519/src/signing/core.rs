...
/// Encodes a namespace length exactly like `commonware_codec::Write for usize` without allocating.
fn namespace_prefix(len: usize) -> ([u8; 5], usize) {
    let mut value = u32::try_from(len).expect("namespace length exceeds u32");
    let mut prefix = [0u8; 5];
    let mut used = 0;
    loop {
        let remaining = value >> 7;
        prefix[used] = (value as u8 & 0x7f) | (u8::from(remaining != 0) * 0x80);
        used += 1;
        if remaining == 0 {
            return (prefix, used);
        }
...
fn challenge_hash(r: &[u8; 32], a: &[u8; 32], message: BatchMessage<'_>) -> [u8; 64] {
    match message {
        #[cfg(test)]
        BatchMessage::Raw(message) => sha512(&[r, a, message]),
        BatchMessage::Namespaced { namespace, message } => {
            let (prefix, used) = namespace_prefix(namespace.len());
            sha512(&[r, a, &prefix[..used], namespace, message])
        }
    }
}
...
fn scalar_phase(
    ...
    let body = |block: usize| {
        ...
        for (j, z) in coefficients.into_iter().enumerate() {
            ...
            let (a_bytes, _, sig, msg) = items[order[i].1 as usize];
            let Some(s) = Scalar::from_canonical_bytes(&sig.s) else {
                valid = false;
                continue;
            };
            let digest = challenge_hash(&sig.r, a_bytes, msg);
...
/// Splits a point-decompression workload into SIMD-unit ranges sized for the
/// supplied strategy.
fn decompression_ranges(count: usize, strategy: &impl Strategy) -> Vec<core::ops::Range<usize>> {
...
/// Decompresses public keys once for retention by the caller's key cache.
fn decompress_verifying_keys_inner<B: Backend>(
    backend: B,
    encodings: &[[u8; 32]],
    strategy: &impl Strategy,
) -> Option<Vec<GAffine>> {
    ...
    let ranges = decompression_ranges(encodings.len(), strategy);
    let mut identity_encoding = [0u8; 32];
    identity_encoding[0] = 1;
    let mut chunks: Vec<(usize, Vec<GAffine>)> = strategy
        .try_map_collect_vec(ranges, |range| {
            let start = range.start * LANES;
            let end = (range.end * LANES).min(encodings.len());
            let mut points = Vec::with_capacity(end - start);
            for unit in range {
                let base = unit * LANES;
                let mut bytes = [identity_encoding; LANES];
                for (lane, encoding) in bytes.iter_mut().enumerate() {
                    if let Some(source) = encodings.get(base + lane) {
                        *encoding = *source;
                    }
                }
                ...
                for point in decompressed
                    .into_iter()
                    .take((encodings.len() - base).min(LANES))
                {
                    points.push(point.ok_or(())?);
                }
            }
            Ok::<_, ()>((start, points))
        })
    ...
    let mut points = Vec::with_capacity(encodings.len());
...
{
    let ranges = decompression_ranges(count, strategy);
    if ranges.is_empty() {
        return Some(Vec::new());
    }
...
/// 3. [`decompress_phase`] over every `R` and each distinct `A` not already retained by the
///    caller's key cache. Cached `A` points become terms directly. A signer's coalesced scalar
///    (the sum of its signatures' `z*h` over a contiguous [`ScalarBlock`] run) is computed once
///    while its term is built.
...
fn verify_batch_inner<B: Backend>(
    ...
    let mut order: Vec<([u8; 32], u32)> = items
        .iter()
        .enumerate()
        .map(|(i, (a_bytes, _, _, _))| (**a_bytes, i as u32))
        .collect();
    ...
    let group_points: Vec<Option<&GAffine>> = groups
        .iter()
        .map(|&(start, end)| {
            (start as usize..end as usize).find_map(|i| items[order[i].1 as usize].1)
        })
        .collect();
    let unresolved_groups: Vec<usize> = group_points
        .iter()
        .enumerate()
        .filter_map(|(group, point)| point.is_none().then_some(group))
        .collect();
    ...
    let resolve_r = |i: usize| {
        let (_, _, sig, _) = items[order[i].1 as usize];
        (sig.r, zr(i))
    };
    ...
    let prepared: Vec<(GAffine, (u32, u32))> = group_points
        .into_iter()
        .zip(groups)
        .filter_map(|(point, group)| point.map(|point| (*point, group)))
        .collect();
    let prepared_terms = strategy.map_collect_vec(prepared, |(point, group)| {
        Term::new(point, &group_scalar(group), width)
    });
    chunks.push(prepared_terms.as_slice());
...
struct DecompressVerifyingKeysCall<'a, S> {
    encodings: &'a [[u8; 32]],
    strategy: &'a S,
}

impl<S: Strategy> WithBackend for DecompressVerifyingKeysCall<'_, S> {
    type Output = Option<Vec<GAffine>>;

    fn call<B: Backend>(self, backend: B) -> Self::Output {
        decompress_verifying_keys_inner(backend, self.encodings, self.strategy)
    }
}

pub(super) fn decompress_verifying_keys(
    encodings: &[[u8; 32]],
    strategy: &impl Strategy,
) -> Option<Vec<GAffine>> {
    with_backend(DecompressVerifyingKeysCall {
        encodings,
        strategy,
    })
}
...
pub(super) fn verify_namespaced_batch<'a>(
    rng: &mut impl CryptoRng,
    ...
    verify_batch(
        ...
        items
            .into_iter()
            .map(|(public_key, point, signature, namespace, message)| {
                (
                    public_key,
                    point,
                    signature,
...
pub(super) fn verify_batch_bytes<'a>(
    ...
    verify_batch(
        rng,
        items.into_iter().map(|(public_key, signature, message)| {
            (public_key, None, signature, BatchMessage::Raw(message))
        }),
        strategy,
    )
}
...
mod tests {
    ...
    #[test]
    fn namespaced_challenge_matches_canonical_union() {
        let r = [7u8; 32];
        let a = [11u8; 32];
        let message = b"message";
        for len in [0, 1, 127, 128, 16_383, 16_384] {
            let namespace = vec![3u8; len];
            assert_eq!(
                challenge_hash(
                    &r,
                    &a,
                    BatchMessage::Namespaced {
                        namespace: &namespace,
                        message,
                    },
                ),
                sha512(&[&r, &a, &union_unique(&namespace, message)]),
            );
        }
    }
    ...
    #[test]
    fn batch_item_does_not_inline_prepared_point() {
        type ReferenceItem<'a> = (
            &'a [u8; 32],
            Option<&'a GAffine>,
            &'a Signature,
            BatchMessage<'a>,
        ...
        assert_eq!(
            core::mem::size_of::<BatchItem<'_>>(),
            core::mem::size_of::<ReferenceItem<'_>>()
        );
    }
