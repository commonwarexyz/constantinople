...
//!   `(payer, receiver, operator, open_nonce)`. Because the address binds the
//!   parties, their identities never need to be stored on-chain.
...
/// `(payer, receiver, operator, open_nonce)` tuple.
///
/// The address is `H(DOMAIN || payer || receiver || operator || open_nonce)`,
/// where `open_nonce` is the nonce of the `OpenChannel` transaction. It lives
/// in the same key space as a regular account, but no private key produces it,
/// so no ordinary transfer can move funds out of the channel — only the channel
...
/// no party's identity needs to be persisted, a settled channel can be
...
/// `receiver`. A channel run by its own payee simply names itself
/// (`operator == receiver`). Because all three participants are inside the
/// hash, a voucher's signature over the address commits to the full tuple:
/// redirecting a voucher to a different payee or settler would need a second
/// preimage.
///
...
pub fn verify_voucher(
    ...
    verify_voucher_key(&key, channel, cumulative, signature)
}

/// Like [`verify_voucher`], but with an already-decoded payer key.
///
/// [`verify_voucher`] decodes then delegates here, so the two stay one
/// predicate; callers that verify repeatedly (the off-chain operator's
/// per-payment path) cache the decoded key and call this directly to skip the
/// per-call curve-point decode.
pub fn verify_voucher_key(
    ...
    payer.verify(VOUCHER_NAMESPACE, &message, signature)
}
...
mod tests {
    ...
    fn channel_address_is_deterministic_and_binds_parties() {
        ...
        let a = channel_address(&payer, &receiver, &operator, 3);
        let b = channel_address(&payer, &receiver, &operator, 3);
        ...
        assert_ne!(a, channel_address(&receiver, &payer, &operator, 3));
        // A different operator yields a different address: the settling key
        // is a participant, not an afterthought.
        assert_ne!(a, channel_address(&payer, &operator, &receiver, 3));
        assert_ne!(a, channel_address(&payer, &receiver, &payer, 3));
        // A different open nonce yields a different address.
        assert_ne!(a, channel_address(&payer, &receiver, &operator, 4));
    }
